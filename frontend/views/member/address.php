...
echo '<link rel="stylesheet" href="/style/address.css" type="text/css">';
        ...
        <!--添加  start-->
            <div class="address_bd mt10">
                ...
                <form action="" name="address_form" id="address_form">
                    <input type="hidden" name="member_id" value="<?php echo Yii::$app->user->identity->getId()?>">
                    <input type="hidden" name="id" value="<?=isset($row->id)?$row->id:''?>" id="is_update" disabled>
                    <ul>
                        <li>
                            <label for="name"><span>*</span>收 货 人：</label>
                            <input type="text" name="name" class="txt" id="name" value="<?=isset($row->name)?$row->name:''?>"/>
                        </li>
                        <li>
                            <label for="area"><span>*</span>所在地区：</label>
                        ...
                        <li>
                            ...
                            <input type="text" name="address" class="txt address"  id="address" value="<?=isset($row->address)?$row->address:''?>"/>
                        </li>
                        <li>
                            <label for="tel"><span>*</span>手机号码：</label>
                            <input type="text" name="tel" class="txt" id="tel" value="<?=isset($row->tel)?$row->tel:''?>"/>
                        </li>
                        <li>
                            <label for="is_default">&nbsp;</label>
                            <input type="checkbox" name="is_default" class="check" id="is_default" value="1" <?=(isset($row->is_default) && $row->is_default==1)?'checked':''?>/>设为默认地址
                        </li>
                        <li>
                            <label for="">&nbsp;</label>
                            <input type="submit" name="" class="btn" value="保存" id="addAddress"/>
                        </li>
                    </ul>
                </form>
            </div>
...
<script type="text/javascript">
    addressInit('cmbProvince', 'cmbCity', 'cmbArea',"<?=isset($row->cmbProvince)?$row->cmbProvince:''?>","<?=isset($row->cmbCity)?$row->cmbCity:''?>","<?=isset($row->cmbArea)?$row->cmbArea:''?>");
</script>
<!--省市县 三级联动 end-->
<!--新增地址 start-->
...
<script type="text/javascript">
        //查看 列表
        $.getJSON('<?=\yii\helpers\Url::to(['member/list'])?>',function (data) {
            var str='';
            $.each(data.data,function () {
                    str+='<dl id="'+this.id+'">';
                        str+='<dt>'+'&emsp;'+this.name+'&emsp;'+this.cmbProvince+'&emsp;'+this.cmbCity+'&emsp;'+this.cmbArea+'&emsp;'+this.address+'&emsp;'+this.tel+'&emsp;'+'</dt>';
                        str+='<dd>';
                str+='<a href="<?=\yii\helpers\Url::to(['member/address'])?>?id='+this.id+'" class="btn-primary">修改</a>';
                            str+='<a href="" class="btn-danger">删除</a>';
                            if (this.is_default){
                                str+='<a href="" class="btn-info">默认地址</a>';
                            }else {
                                str+='<a href="" class="btn-info">设为默认地址</a>';
                            }
                        str+='</dd>';
                    str+='</dl>';
            });
            $('.address_hd').append(str);
            $('dl:last').addClass('last');

        });

        //新增 修改
        $('.address_bd').on('click','#addAddress',function () {
            var form=[];
            var id=$('#is_update').val();
            if (!id){
                //新增
                form=$('#address_form').serializeArray();
                $.post('<?=\yii\helpers\Url::to(['member/add-address'])?>',{'form':form},function (data) {
                    if (data.data){
                        console.debug(data.data);
                        var html='';
                        html+='<dl class="last" id="'+data.id+'">';
                        html+='<dt>&emsp;'+data.data['name']+'&emsp;'+data.data['cmbProvince']+'&emsp;'+data.data['cmbCity']+'&emsp;'+data.data['cmbArea']+'&emsp;'+data.data['address']+'&emsp;'+data.data['tel']+'&emsp;'+'&emsp;'+'</dt>';
                        html+='<dd>';
                        html+='<a href="" class="btn-primary">修改</a>';
                        html+='<a href="" class="btn-danger">删除</a>';
                        if (data.data['is_default']){
                            $('.btn-info').text('设为默认地址');
                            html+='<a href="" class="btn-info">默认地址</a>';
                        }else{
                            html+='<a href="" class="btn-info">设为默认地址</a>';
                        }
                        html+='</dd>';
                        html+='</dl>';
                        $('.content').find('dl:last').removeClass('last');
                        $('.address_hd').append(html);
                        $('#address_form :text').val('');
                    }
                },'json');
            }else {
                //修改
                form=$('#address_form').serializeArray();
                $.post('<?=\yii\helpers\Url::to(['member/edit-address'])?>',{'id':id,'form':form},function (data) {
                    if (data.data){
                        var html='';
                        html+='<dt>&emsp;'+data.data['name']+'&emsp;'+data.data['cmbProvince']+'&emsp;'+data.data['cmbCity']+'&emsp;'+data.data['cmbArea']+'&emsp;'+data.data['address']+'&emsp;'+data.data['tel']+'&emsp;'+'&emsp;'+'</dt>';
                        html+='<dd>';
        ...
        $('.content').on('click','.btn-danger',function () {
            if(confirm('确定删除该地址?')){
                ...
                $.getJSON('<?php echo \yii\helpers\Url::to(['member/del-address'])?>',{"id":id},function (data) {
                    if(data.status){
                        $('#'+id).remove();
                        alert('删除成功');
                    }
        ...
        //设为默认地址
        $('.ml10').on('click','.btn-info',function () {
            if(confirm('确定设置该默认地址?')){
                var id=$(this).closest('dl').attr('id');
                $.getJSON('<?php echo \yii\helpers\Url::to(['member/edit-default'])?>',{"id":id});
                $('.btn-info').text('设为默认地址');
                $(this).text('默认地址');
            }
            return false;
